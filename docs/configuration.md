...
agy (Antigravity CLI) is likewise verified for crewmate and scout launches ONLY; a secondmate is refused, but not for a config-scoping reason - an isolated `$HOME` is verified to relocate agy's whole config/credential tree, the same technique muse and gemini already use - the refusal instead rests on the unverified question of whether agy's documented `Stop` hook actually fires for a firstmate-shaped `-i` launch; [`docs/verification/agy.md`](verification/agy.md) owns that evidence and the exact follow-up needed to lift the refusal.
